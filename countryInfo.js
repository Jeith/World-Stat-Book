...
function loadPage(){
    ...
    function updateUISuccess(response){
        ...
        // Government tab
        var govType = response.Government["Government type"].text;
        console.log(govType);

        var governmentType = document.getElementById("governmentType");
        governmentType.textContent = govType;

        var adminDiv = response.Government["Administrative divisions"].text;
        // var adminDiv2 = response.Government["Administrative divisions"].note.text;
        
        var adminDivision = document.getElementById("adminDivision");
        adminDivision.textContent = adminDiv;

        // if(adminDiv2.length > 0){
        //     adminDivision.textContent = adminDiv + ". " + adminDiv2;
        // }
        // else{
        //     adminDivision.textContent = adminDiv;
        // }

        var indep = response.Government.Independence.text;
        var independence = document.getElementById("independence");
        independence.textContent = indep;

        var legSys = response.Government["Legal system"].text;
        var legalSystem = document.getElementById("legalSystem");
        legalSystem.textContent = legSys;

        var natAnt = response.Government["National anthem"].name.text;
        var nationalAnthem = document.getElementById("nationalAnthem");
        nationalAnthem.textContent = natAnt;


        // Flag
        var flagUrl = "https://www.countryflags.io/" + cc + "/flat/64.png";
        var flag = document.getElementById("flagIcon");
